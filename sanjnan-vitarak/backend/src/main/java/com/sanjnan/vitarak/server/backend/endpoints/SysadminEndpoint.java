...
public class SysadminEndpoint {
    ...
    public List<Actionable> getActionables(User user) throws ForbiddenException, OAuthRequestException {
        ...
        if (accounts.size() > 0) {
            ...
            Actionable actionable = new Actionable(SanjnanActionTypes.APPROVE_BUSINESS_USER,
                    SanjnanStringCodes.APPROVE_BUSINESS_USER_TITLE,
                    SanjnanStringCodes.APPROVE_BUSINESS_USER_DESCRIPTION,
                    accounts.size(),
                    false);

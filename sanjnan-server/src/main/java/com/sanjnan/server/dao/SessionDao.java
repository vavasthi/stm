...
public interface SessionDao extends CrudRepository<SessionEntity, UUID> {
  ...
  @Query("SELECT se from  com.sanjnan.server.entity.SessionEntity se where se.authToken = :authToken")
  List<SessionEntity> findByAuthToken(@Param("authToken") String authToken);

  @Query("SELECT se from  com.sanjnan.server.entity.SessionEntity se where se.remoteAddress = :remoteAddress")

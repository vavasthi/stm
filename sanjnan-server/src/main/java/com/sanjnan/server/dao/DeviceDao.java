...
public interface DeviceDao extends CrudRepository<DeviceEntity, UUID> {
  ...
  @Query("SELECT de from  com.sanjnan.server.entity.DeviceEntity de where de.deviceRegistrationId = :deviceRegistrationId")
  List<DeviceEntity> findByDeviceRegistrationId(@Param("deviceRegistrationId") String deviceRegistrationId);

  @Query("SELECT de from  com.sanjnan.server.entity.DeviceEntity de where de.sessionEntity = :sessionEntity")
